...
def login():
    if request.method == 'POST':
        email = request.form['email']
        password = request.form['password']  # This will be ignored now
        ...
        if data:
            # User exists, so redirect to upload
...
def prediction():
    ...
    if request.method == 'POST':
        ...
        missed_bytes = float(request.form['missed_bytes'])
        history = float(request.form['history'])
        orig_pkts = float(request.form['orig_pkts'])
        orig_ip_bytes = float(request.form['orig_ip_bytes'])
        resp_pkts = float(request.form['resp_pkts'])
        resp_ip_bytes = float(request.form['resp_ip_bytes'])
